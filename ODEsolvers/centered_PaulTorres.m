...
%using the Centered Difference scheme on a uniform mesh of N grid points.
%--------------------------------------------------------------------------
...
L(1,1) = -(2-h*h*Q(x(2)));
L(1,2) = 1+.5*h*P(x(2));
L(N-1,N-2) = 1-.5*h*P(x(N));
L(N-1,N-1) = -(2-h*h*Q(x(N)));

b = zeros(N-1,1);
b(1,1) = h*h*R(x(2))-(1-.5*h*P(x(2)))*alpha;
b(N-1,1) = h*h*R(x(N))-(1+.5*h*P(x(N)))*beta;

for i = 2:N-2
    L(i,i) = -(2-h*h*Q(x(i+1)));
    L(i,i+1) = 1+.5*h*P(x(i+1));
    L(i,i-1) = 1-.5*h*P(x(i+1));
